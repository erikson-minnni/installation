...
hx = SimpleHX711(20, 21, REF_UNIT, OFFSET_VAL)
...
def tare():
    ...
    hx.zero()
    readings.clear()  # Wipe history so old weights don't leak into new readings
    print("Tare complete!\n")
...
def scale():
    ...
    # Track the last 3 samples for whole-gram accuracy
    readings = deque(maxlen=3)
    
    print("Zero-CPU scale loop active...")
    
    while True:
        try:
            # --- THE FIX ---
            # Instead of blindly calling hx.weight() and trapping the CPU,
            # we check if the chip has data ready. If it doesn't, Python takes 
            # a micro-nap (yielding 100% of the CPU back to the Pi) and checks again.
            while not hx.isReady():
                time.sleep(0.005)  # 5ms nap while waiting for hardware
            
            # Because we know data is 100% ready, this call returns INSTANTLY 
            # without spinning the CPU at all!
            single_read = hx.weight(1)
            readings.append(float(single_read))
            
            # Average out our rolling window
            if len(readings) > 0:
                avg_weight = sum(readings) / len(readings)
                rounded_weight = round(avg_weight)
            else:
                rounded_weight = 0

            # Noise gate check
            current_scale_weight = 0 if rounded_weight < 3 else rounded_weight
            
            # Post-read breathing room before the next check loop
            time.sleep(0.01)
       #     print (current_scale_weight)

        except Exception as e:
            print(f"Scale loop glitch: {e}")
            time.sleep(0.5)
